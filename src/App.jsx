...
const ACTION_OPTS   = ["", "Pending", "Pending Save", "Welcome Call", "Onboarding Call", "Pending Agent Assist", "Serviced", "Save", "UW Action Needed", "UW Action Resolved", "Lost", "Hang up"];
...
async function updateInboundOnSheet(data) {
  try {
    const fd = new URLSearchParams();
    fd.append("recordType","inboundUpdate");
    Object.entries(data).forEach(([k, v]) => fd.append(k, v ?? ""));
    await fetch(GOOGLE_SHEET_WEB_APP_URL, { method:"POST", mode:"no-cors", body:fd });
  } catch (e) { console.error("Inbound update failed:", e); }
}
async function deleteInboundOnSheet(data) {
  try {
    const fd = new URLSearchParams();
    fd.append("recordType","inboundDelete");
